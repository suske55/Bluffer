...
public class ConnectionHandler<T> {
	...
	protected final AsyncTBGProtocol _protocol;
	...
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key) {
		_sChannel = sChannel;
		_data = data;
		_protocol = (AsyncTBGProtocol)_data.getProtocolMaker().create();
		_tokenizer = (FixedSeparatorMessageTokenizer)_data.getTokenizerMaker().create();
		_skey = key;
	}
